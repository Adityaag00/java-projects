...
public class Day3 {
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (int i = 0; i < inputs.size(); i++) {
            for (int j = 0; j < inputs.get(i).length(); j++) {
                if (!(Character.isDigit(inputs.get(i).charAt(j)) || inputs.get(i).charAt(j) == '.')) {
    ...
    public static int visitAllSides(int x, int y, List<String> strings) {
        int sum = 0;
        sum += getValueAdjacent(strings, Direction.UP, x, y);
        sum += getValueAdjacent(strings, Direction.DOWN, x, y);
        sum += getValueAdjacent(strings, Direction.LEFT, x, y);
        sum += getValueAdjacent(strings, Direction.RIGHT, x, y);
        sum += getValueAdjacent(strings, Direction.UP_LEFT, x, y);
        sum += getValueAdjacent(strings, Direction.UP_RIGHT, x, y);
        sum += getValueAdjacent(strings, Direction.DOWN_LEFT, x, y);
        sum += getValueAdjacent(strings, Direction.DOWN_RIGHT, x, y);
        return sum;
    }
